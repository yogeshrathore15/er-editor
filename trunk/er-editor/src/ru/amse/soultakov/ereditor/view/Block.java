...
public abstract class Block implements IViewable
{
	...
	protected void drawSelection(Graphics2D graphics)
	{
		...
		{
			// left up
			drawSelectionSquare(graphics, -SELECTION_SQUARE_SIZE / 2, -SELECTION_SQUARE_SIZE / 2);
			// left middle
			drawSelectionSquare(graphics, -SELECTION_SQUARE_SIZE / 2, getHeight() / 2
					- SELECTION_SQUARE_SIZE / 2);
			// left down
			drawSelectionSquare(graphics, -SELECTION_SQUARE_SIZE / 2, getHeight()
					- SELECTION_SQUARE_SIZE / 2);
			// down
			drawSelectionSquare(graphics, getWidth() / 2 - SELECTION_SQUARE_SIZE / 2, getHeight()
					- SELECTION_SQUARE_SIZE / 2);
			// right down
			drawSelectionSquare(graphics, getWidth() - SELECTION_SQUARE_SIZE / 2, getHeight()
					- SELECTION_SQUARE_SIZE/2);
			// right middle
			drawSelectionSquare(graphics, getWidth() - SELECTION_SQUARE_SIZE / 2, getHeight() / 2
					- SELECTION_SQUARE_SIZE / 2);
			// right up
	...
	private void drawSelectionSquare(Graphics2D graphics, int x, int y)
	{
		...
		graphics.fillRect(getX() + x, getY() + y, SELECTION_SQUARE_SIZE, SELECTION_SQUARE_SIZE);
	...
	{
		return "[X = " + getX() + ", Y = " + getY() + ", W = " + getWidth() + ", H = "
				+ getHeight() + "]";
	}
	...
	{
		graphics.setColor(Color.BLACK);
		graphics.fillRoundRect(getX() + 2, getY() + 2, getWidth(), getHeight(), 10, 10);
	}
