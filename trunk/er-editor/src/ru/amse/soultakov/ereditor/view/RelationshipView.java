...
public class RelationshipView extends Line
{
	...
	public void paint(Graphics2D graphics)
	{
		...
		else if ((se.getX() >= fe.getX()) && (se.getX() <= fe.getX() + fe.getWidth()))
		{
	...
	private void drawSelection(Graphics2D graphics)
	{
		if (isSelected())
		{
			...
			graphics.fillRect(firstCenterX - SELECTION_RECTANGLE_SIZE/2, firstCenterY
					- SELECTION_RECTANGLE_SIZE/2, SELECTION_RECTANGLE_SIZE, SELECTION_RECTANGLE_SIZE);
			graphics.fillRect(secondCenterX - SELECTION_RECTANGLE_SIZE/2, secondCenterY
					- SELECTION_RECTANGLE_SIZE/2, SELECTION_RECTANGLE_SIZE, SELECTION_RECTANGLE_SIZE);
			graphics.setColor(Color.BLACK);
			graphics.drawRect(firstCenterX - SELECTION_RECTANGLE_SIZE/2, firstCenterY
					- SELECTION_RECTANGLE_SIZE/2, SELECTION_RECTANGLE_SIZE, SELECTION_RECTANGLE_SIZE);
			graphics.drawRect(secondCenterX - SELECTION_RECTANGLE_SIZE/2, secondCenterY
					- SELECTION_RECTANGLE_SIZE/2, SELECTION_RECTANGLE_SIZE, SELECTION_RECTANGLE_SIZE);
		}
	}
	...
	private void drawBothRight(Graphics2D graphics, int firstY, int secondY)
	{
		...
		Point firstPoint = paintRelationshipEnd(graphics, firstX + END_LENGTH, firstY, firstX,
				firstY, relationship.getFirstEnd().getMultiplicity(), true);
		Point secondPoint = paintRelationshipEnd(graphics, secondX, secondY, secondX + END_LENGTH,
				secondY, relationship.getSecondEnd().getMultiplicity(), false);
	...
	private void drawFirstRightSecondLeft(Graphics2D graphics, int firstY, int secondY)
	{
		int firstX = getFirstEntityView().getX() + getFirstEntityView().getWidth();
		int secondX = getSecondEntityView().getX();
		Point firstPoint = paintRelationshipEnd(graphics, firstX, firstY, firstX + END_LENGTH,
				firstY, relationship.getFirstEnd().getMultiplicity(), false);
		Point secondPoint = paintRelationshipEnd(graphics, secondX - END_LENGTH, secondY, secondX,
				secondY, relationship.getSecondEnd().getMultiplicity(), true);
		drawMainLine(graphics, firstPoint, secondPoint);
	}
	...
	private void drawMainLine(Graphics2D graphics, Point firstPoint, Point secondPoint)
	{
		...
		drawLine(graphics, firstPoint.x, firstPoint.y, xCenter, yCenter, relationship
				.getSecondEnd().getMultiplicity());
		drawLine(graphics, secondPoint.x, secondPoint.y, xCenter, yCenter, relationship
				.getSecondEnd().getMultiplicity());
	}

	private Point paintRelationshipEnd(Graphics2D graphics2D, int x1, int y1, int x2, int y2,
			RelationshipMultiplicity mult, boolean left2Right)
	{
