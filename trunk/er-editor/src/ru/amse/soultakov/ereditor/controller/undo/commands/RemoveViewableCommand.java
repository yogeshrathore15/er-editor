...
public class RemoveViewableCommand implements ICommand
{
	...
	public RemoveViewableCommand(final DiagramEditor diagramEditor, final Set<IViewable> viewables)
	{
	...
	private void removeRelationship(IViewable v)
	{
		...
		{
			removeFKRelationshipEnd(v,(FKRelationshipEnd) rv.getRelationship().getFirstEnd());
		}
		else if (rv.getRelationship().getSecondEnd() instanceof FKRelationshipEnd)
		{
			removeFKRelationshipEnd(v,(FKRelationshipEnd) rv.getRelationship().getSecondEnd());
		}
	}
	...
	private void restoreEntityAttributes(IViewable v)
	{
		if (v instanceof RelationshipView)
		{
			...
			if (rv.getRelationship().getFirstEnd() instanceof FKRelationshipEnd)
			{
				entity = ((FKRelationshipEnd) rv.getRelationship().getFirstEnd()).getEntity();
			}
			else if (rv.getRelationship().getSecondEnd() instanceof FKRelationshipEnd)
			{
				entity = ((FKRelationshipEnd) rv.getRelationship().getSecondEnd()).getEntity();
			}
			else
			{
				return;
